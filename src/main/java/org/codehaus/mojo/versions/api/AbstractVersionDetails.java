    ...
    implements VersionDetails
{
                                                   ...
                                                   boolean includeLower, boolean includeUpper )
    {
        ...
        while ( i.hasNext() )
        {
			...
			//Changed by Yongjun Rong
			int lower = lowerBound == null || (!includeSnapshots && ArtifactUtils.isSnapshot( lowerBound.toString())) ? -1 : versionComparator.compare( lowerBound, candidate );
            int upper = upperBound == null || (!includeSnapshots && ArtifactUtils.isSnapshot( upperBound.toString())) ? +1 : versionComparator.compare( upperBound, candidate );
			//End Yongjun Rong
            if ( lower > 0 || upper < 0 )
            {
                continue;
